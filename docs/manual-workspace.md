...
`Doc_Update`（編集可否）は**ワークスペースのロールのみ**で決まり（`buildDocPermissions(role)`、`isWriter = isAdmin || member || write`）、Reader ロールのユーザーは `Doc_Update = false` になる。

⚠️ **ただしサーバー Admin（`is_admin`）は `getDocRole` をバイパスする**（`permission.service.ts` の「運用上の最終手段」）。
そのため **Admin だけはマニュアルを編集できていた**（2026-09-18 に利用者が発見）。
以前ここには「Admin はバイパスしない」と書いてあったが、**誤りだった**。

編集できても、マニュアルは更新のたびにワークスペースごと作り直されるため、
**その編集は次の更新で黙って消える**。それなら最初から書かせないほうがよいので、
`getDocRole` の先頭で「マニュアルWS（ID 先頭 `ffffffff-ffff-4fff-bfff-`）は Reader 止まり」にした（#241）。
...
| 案 | 判定 | 理由 |
|----|:----:|------|
| 共有WS内の1ドキュメントを保護（#66） | ✗ | #66 は advisory。Owner/Admin がトグル解除可。doc単位ロールは非機能 |
...
⚠️ **日本語の docId は変えないこと**（本番の履歴とリンクが切れる）。
英語版は #241 で新しく採番した。

| docId | ページ |
|-------|--------|
| `vewJ1Zh9N2_rxVCRXhHum` | [JA] 👋 はじめにお読みください（目次） |
...
### 4.4-b 日英併記（#241・2026-09-18）

マニュアルは**1つのワークスペースに日本語と英語の両方**を入れる。
言語ごとにワークスペースを分けない（シードもリセットも配布も二重になり、維持できない）。

- 表示名は **`📖 Manual`**（`ManualWorkspaceService.WORKSPACE_NAME`）
- 題の頭に `[JA]` / `[EN]` を付ける。並べたときに言語でまとまる
- 目次は日英それぞれにあり、冒頭で互いにリンクする
- 章立ては日英で同じ（7章）

⚠️ **UI を英語にしても本文は英語にならない。** 翻訳されるのはメニューやボタンだけで、
本文は保存された文章そのもの（2026-09-18 に実機で確認）。だから英語のページを実体として持つ。

### 4.4-c 並び順と内部データ（#241・2026-09-18）

⚠️ **一覧の既定の並びは「更新日時の降順」**（`all-page.tsx`）。
取り込みが書き出し元の日時を捨てていたため、章の順番が取り込みの実行順になり、
バラバラに見えていた。取り込みは日時を引き継ぐようにし、seed 側は
**上から読む順に並ぶ日時**を焼き込んである（目次 → JA 1〜7 → EN 目次 → EN 1〜7）。

⚠️ **内部データに台帳の行を作らない。** ワークスペース自身の doc と
