...
### 始業時など、同時ログインが集中すると遅い・タイムアウトする

**症状**: 朝の始業時など多人数が一斉にログインすると、後の利用者ほど
サインインに時間がかかる。ひどいときはタイムアウトする。
1人ずつなら速いのに、重なると遅い。

**原因**: パスワード照合（bcrypt）は意図的に重い処理で、1回あたり
**約 300ms** かかります。これは総当たり攻撃を防ぐための仕様で、速くはできません。

問題は**同時に処理できる本数**です。Node はこうした重い処理を
バックグラウンドのスレッドプールで捌きますが、その既定値は
**4 本**（Node/libuv のハードコード。CPU コア数とは無関係）です。
16 コアのサーバーでも、既定のままでは**同時 4 件しか照合できません**。

| 同時ログイン | 最後の利用者の待ち時間（プール=4 の場合） |
|---|---|
| 4 件 | 約 0.3 秒 |
| 16 件 | 約 1.2 秒 |
| 32 件 | **約 2.5 秒** |

さらに、このプールは**ファイル I/O や gzip 圧縮とも共有**されます。
画像アップロードの最中や、夜間バックアップの圧縮中は、ログインが
いっそう遅くなります。

**対処**: ofuro-wiki は起動時に **CPU コア数から自動で決める**ため、
通常は設定不要です（下限 4・上限 16）。起動ログで確認できます。

```bash
docker compose logs app | grep スレッドプール
# → スレッドプール: 8（同時に処理できる照合数）
```

自動で決めた値を変えたい場合のみ、`.env` に設定します（再起動が必要）。

```bash
UV_THREADPOOL_SIZE=16
```

> **⚠️ この設定は `.env` を読み込む前に決まる**ため、アプリが `.env` を
> 直接先読みしています。値を変えたら**起動ログで反映を確認してください。**

> **⚠️ コア数を大きく超える値にしても速くなりません。**
> bcrypt は CPU を使い切る処理のため、スレッドを増やしても
> 待ち行列が伸びるだけです。まず起動ログの値と `nproc` を比べてください。

> **⚠️ それでも足りない場合は、台数を増やす方向で考えてください。**
> 1台あたりの同時ログイン処理数は CPU コア数が上限です。
