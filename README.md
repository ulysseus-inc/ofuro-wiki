...
- **管理画面 →「サーバー設定」→「リンクプレビュー・リッチ埋め込み」を ON** にすると、ユーザーが貼り付けた URL について**サーバーがその外部サイトへ接続**してメタデータ・画像を取得します（＝ユーザー操作起因の外部送信）。SSRF 対策（内部アドレス遮断・リダイレクト再検証・サイズ/タイムアウト上限）を実装しています。
