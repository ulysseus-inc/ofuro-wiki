import { Path } from '@ofuro-tools/utils/path';
import { execAsync } from '@ofuro-tools/utils/process';
import type { Package, PackageName } from '@ofuro-tools/utils/workspace';
...
export class RunCommand extends PackageCommand {
  ...
  static override usage = PackageCommand.Usage({
    description: 'ofuro-wiki Monorepo scripts',
    details: `
      \`ofuro web <script>\`    Run any script defined in package's package.json

      \`ofuro init\`            Generate the required files if there are any package added or removed

      \`ofuro clean\`           Clean the output files of ts, cargo, webpack, etc.

      \`ofuro bundle\`          Bundle the packages

      \`ofuro build\`           A proxy for <-p package>'s \`build\` script

      \`ofuro dev\`             A proxy for <-p package>'s \`dev\` script
    `,
    examples: [
      [`See detail of each command`, '$0 -h'],
      [
        `Run custom 'xxx' script defined in @ofuro/web's package.json`,
        '$0 web xxx',
      ],
      ...
      [`Run 'build' script for @ofuro/web`, '$0 build -p web'],
      [
        `Run 'build' script for @ofuro/web with all deps prebuild before`,
        '$0 build -p web --deps',
      ],
    ],
  });
  ...
  async runScript(
    ...
    const isOfuroCommand = args[0] === 'ofuro';
    if (isOfuroCommand) {
      // remove 'ofuro' from 'ofuro xxx' command
      args.shift();
