import { type Path, ProjectRoot } from '@ofuro-tools/utils/path';
...
const CA_ORG = 'ofuro-wiki Dev CA Self Signed Org';
const CA_NAME = 'ofuro-wiki Dev CA Self Signed CN';
...
export class CertCommand extends Command {
  ...
  private createCert(domain: string) {
    if (!this.checkInstalled(CA_PEM_PATH)) {
      this.logger.error(
        'CA not installed. Please run `yarn ofuro cert --install` first.'
      );
      process.exit(1);
    }
  ...
  private trustCa(pem: Path) {
    this.logger.info(`Trusting ofuro-wiki Dev Self Signed CA`);
    this.exec(
