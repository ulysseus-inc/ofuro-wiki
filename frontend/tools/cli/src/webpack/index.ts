...
import { getBuildConfig } from '@ofuro-tools/utils/build-config';
import { Path, ProjectRoot } from '@ofuro-tools/utils/path';
...
export function createHTMLTargetConfig(
  ...
  const config: webpack.Configuration = {
    ...
    devtool: buildConfig.debug ? 'eval-cheap-module-source-map' : 'source-map',
    resolve: {
      symlinks: true,
      extensionAlias: {
        '.js': ['.js', '.tsx', '.ts'],
          ...
          patterns: [
            {
              ...
              from: new Package('@ofuro/core').join('public').value,
            },
          ],
        }),
      // S3 upload plugin removed (ofuro-wiki)
      !buildConfig.debug &&
        process.env.PERFSEE_TOKEN &&
        new PerfseePlugin({ project: 'ofuro-wiki' }),
