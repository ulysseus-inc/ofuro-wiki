...
export const defaultSlashMenuConfig: SlashMenuConfig = {
  items: () => {
    ...
    const muT = translateSlashItem('Move Up', 'Shift this line up.');
    const mdT = translateSlashItem('Move Down', 'Shift this line down.');
    const cpT = translateSlashItem('Copy', 'Copy this line to clipboard.');
    const dupT = translateSlashItem('Duplicate', 'Create a duplicate of this line.');
    const delT = translateSlashItem('Delete', 'Remove this line permanently.');

    return [
      {
        name: translateSlashItem('Today').name,
        icon: TodayIcon(),
        tooltip: slashMenuToolTips['Today'],
        description: formatDate(now),
        group: translateGroupStr('6_Date@0'),
        action: ({ std, model }) => {
          insertContent(std, model, formatDate(now));
        },
      },
      {
        name: translateSlashItem('Tomorrow').name,
        icon: TomorrowIcon(),
        tooltip: slashMenuToolTips['Tomorrow'],
        description: formatDate(tomorrow),
        group: translateGroupStr('6_Date@1'),
        action: ({ std, model }) => {
          const tomorrow = new Date();
          tomorrow.setDate(tomorrow.getDate() + 1);
      ...
      {
        name: translateSlashItem('Yesterday').name,
        icon: YesterdayIcon(),
        tooltip: slashMenuToolTips['Yesterday'],
        description: formatDate(yesterday),
        group: translateGroupStr('6_Date@2'),
        action: ({ std, model }) => {
          const yesterday = new Date();
          yesterday.setDate(yesterday.getDate() - 1);
      ...
      {
        name: translateSlashItem('Now').name,
        icon: NowIcon(),
        tooltip: slashMenuToolTips['Now'],
        description: formatTime(now),
        group: translateGroupStr('6_Date@3'),
        action: ({ std, model }) => {
      ...
      },
      {
        ...
        group: translateGroupStr('8_Actions@0'),
        action: ({ std, model }) => {
          const { host } = std;
      ...
      },
      {
        ...
        group: translateGroupStr('8_Actions@1'),
        action: ({ std, model }) => {
          const { host } = std;
      ...
      },
      {
        ...
        group: translateGroupStr('8_Actions@2'),
        action: ({ std, model }) => {
      ...
      {
        name: dupT.name,
        description: dupT.description,
        icon: DualLinkIcon(),
        tooltip: slashMenuToolTips['Copy'],
        group: translateGroupStr('8_Actions@3'),
        action: ({ std, model }) => {
      ...
      },
      {
        ...
        group: translateGroupStr('8_Actions@4'),
        action: ({ std, model }) => {
