...
const deleteSelectedText = (doc: Store, textSelection: TextSelection) => {
  ...
  if (textSelection.to) {
    ...
    if (firstBlock.text) {
      const delLen = Math.max(0, firstBlock.text.length - startOffset);
      firstBlock.text.delete(startOffset, delLen);
    }

    // Delete text from the beginning to endOffset in the last block
    if (lastBlock.text) {
      const toIndex = Math.min(textSelection.to.index, lastBlock.text.length);
      const toLength = Math.max(
        0,
        Math.min(textSelection.to.length, lastBlock.text.length - toIndex)
      );
      lastBlock.text.delete(toIndex, toLength);
    }
    ...
    if (firstBlock.text) {
      const fromLength = Math.max(
        0,
        Math.min(textSelection.from.length, firstBlock.text.length - startOffset)
      );
      firstBlock.text.delete(startOffset, fromLength);
    }
  }
