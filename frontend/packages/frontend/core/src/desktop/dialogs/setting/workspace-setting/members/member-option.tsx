import { MenuItem, notify, useConfirmModal } from '@ofuro/component';
import {
  type Member,
  WorkspaceMembersService,
  WorkspacePermissionService,
} from '@ofuro/core/modules/permissions';
import { Permission, WorkspaceMemberStatus } from '@ofuro/graphql';
...
export const MemberOptions = ({
      ...
      .then(result => {
        if (result) {
          notify.success({
            title: t['com.affine.payment.member.team.change.notify.title'](),
            message: t[
              'com.affine.payment.member.team.change.member.notify.message'
            ]({
              name: member.name || member.email || member.id,
            }),
          });
  ...
  const handleChangeToReader = useCallback(() => {
    membersService
      .adjustMemberPermission(member.id, Permission.Read)
      .then(result => {
        if (result) {
          notify.success({
            title: t['com.affine.payment.member.team.change.notify.title'](),
            message: t[
              'com.affine.payment.member.team.change.reader.notify.message'
            ]({
              name: member.name || member.email || member.id,
            }),
          });
