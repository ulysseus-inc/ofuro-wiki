...
import { useAdminSettingList } from '../admin-setting';
...
export const SettingSidebar = ({
  ...
  const adminSettingList = useAdminSettingList();
  ...
  const groups = useMemo(() => {
    ...
    if (adminSettingList.length > 0) {
      sections.push({
        key: 'setting:admin',
        title: t['com.affine.settingSidebar.settings.admin'](),
        items: adminSettingList,
      });
    }
    const res = sections.map(group => {
      return {
        ...group,
    ...
    return res;
  }, [activeTab, adminSettingList, generalList, gotoTab, t, workspaceSettingList]);
