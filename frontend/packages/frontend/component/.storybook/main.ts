...
export default {
  ...
  async viteFinal(config, _options) {
    const { getBuildConfig } = await import('@ofuro-tools/utils/build-config');
    const { Package } = await import('@ofuro-tools/utils/workspace');
    ...
    return mergeConfig(config, {
      ...
      define: Object.entries(
        getBuildConfig(new Package('@ofuro/web'), {
          mode: 'development',
          channel: 'canary',
        })
